...
class UserClass { // Имя не должно совпадать с именем Конструктора
    ...
    constructor (name, age) { // Используем ключевое слово constructor и в скобках переменные - это если нам нужно обрабатывать вводимые извне переменные. Сам constructor() является методом, как будто это  myInfo () который ниже.
        this.name = name
        this.age = age
    } // , здесь мога быть запятая как в объекте которая бы разделяла методы, но в классе этого не нужно делать. Можно точку с запятой ; но ни в коем случае не просто запятую 
        myInfo () { // для класса используется создание метода как для обычного объекта. В constructor его нет необходимости помещать так как он не оиспользует воодимые данные извне
            console.log("Меня зовут "+this.name+", мне "+ this.age + " лет")}
