...
obj["name"] = "Ivan" /* присвоили новый ключ и значение, причем, в отличии от массивов, тут не нужно соблюдать числовую последовательность индексов */

console.log(obj) 
// Получили: {color: 'Tomato', numbers: Array(2), checked: true, name: 'Ivan'}
...
/* Вопрос: 
Заполните пропуски
Объект на JavaScript состоит из пары: ключ и значение */
...
// Получили: Второе всплывающее окно в Котором написано "Введите ващ возраст" и ниже поле для ввода текста с мигающим курсором, и ниже 2 кнопки "Продолжить" и "Отмена". После введения возраста и нажатия любой кноки "продолжить" или "Отмена" окно исчезает, при этом введеннные значения никуда не записываются и не запоминаются. Чтобы они запомнились нужно использовать присвоение переменной, о чем далее.
...
// Зачем выводить null если пользователь ничего не ввел? Это - бред. Для того чтобы этого не было нужно использовать условие if  
