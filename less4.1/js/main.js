...
// (max + 1 - min) используется для того, чтобы намеренно прибавить +1 и получить при пределах 1-5 0,98(теоретический выскочивший рандом)*5=4.9 который потом плюсуется с мин = 1, получаем 5.9, и округляем вниз до 5. То есть этот + 1 даёт боьшую вероятность получить ровно 5, иначе если +1 не написать, то 5 по стандартному решению через random будет выпадать крайне редко. Есть простой метод округления без всяких +1, округление простым round-ом, но тогда там будут проблемы с низкой веротностью получения крайних значений, получится неравномерная характеристика вероятностей.
...
console.log("\n Длина массива arr.length = " + arr.length)

console.log("\n Выведем элемент 2 массива. arr[2] = " + arr[2])

arr[2] = "Text" /* примечание: Не нужно писть let если хотим присвоить новое значение индексу массива. Если написать let то пишет кучу ошибок в стиле невозможно присвоить объект */
...
console.log("\n Массив arr:" + arr)
console.log(arr) /* видим, что "Str1" поменялось на "Text" */
