...
let obj3 = {
    ...
    hello () { // создали функцию с названием hello, при этом function писать не надо
        console.log(`Меня зовут ${this.name}`) // с помощью this. мы можем обратиться к ключу внутри объекта в котором находимся, причем результатом будет значение этого ключа, но будет результат только если вызвать этот метод отдельным кодом.
    },
}
