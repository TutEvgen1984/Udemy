...
// console.log(`i = ${i}`) результат "i is not defined" то есть i в цикле for используется там временно и только во время цикла, и потом удаляется из памяти
...
//     // console.log(arr_for_circleFOR[item]*=2) // а так вообще сплошные ошибки NaN потому что item это сам элемент, а не индекс. 
// }
// console.log(`arr_for_circleFOR = ${arr_for_circleFOR}`)
...
let filter2 = people.filter(function(item) {
    if(item.id<3) return item /* теперь функция возвращает найденнt элементs, и в переменной получаем несколько массивов */
})
console.log(filter2)
