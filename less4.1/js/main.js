...
function randomNumber() {
    ...
    for (let attempt=1; attempt<=10; attempt++) { // 10 попыток
        let user_number = +prompt(`Попытка № ${attempt}, Ввведите число:`) // +prompt означает превращаем введенную пользователем строку в число
        if (user_number == random_number) {
            return alert (`Вы угадали число! Попыток: ${attempt}, Число: ${user_number}`) // ретёрн завершит цикл
        ...
        // else return alert (`Вы не угадали число. Загаданное число было: ${random_number}`) // эта строка не сработает в самом конце если не угадали число, потому что сработало выносим её за цикл for
    }
...
let date_now = new Date () // Создали новый объект класса Date. Это из ООП который мы ещё не изучали)
...
// это 60 000 милисекунд, 60 секунд = 1 минута с начала исчесления времени 1 января 1970 года
...
console.log(d_1)
// месяц и день нужно указывать 2-мя числами. 
// T - это для разделения даты и времени.
// если необходимо, то ставим точку и указываем 3-значное количество миллисекунд вплоть до 999
