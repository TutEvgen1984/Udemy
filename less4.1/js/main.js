...
// Код на основе стандартных if-else:
// if (age < 18){
...
while (count < 10) {
    console.log(count) /* получили в консоли 10 строк с цифрами от 0 до 4 */
    ...
    // Если прибавление не написать, по бесконечно выводятся цифры в консоль влоть до миллионов, и проессор загружается почти на 100% и браузер тоже виснет. Это потому что count всегда остается 0 и не изменяется, и выполняется цикл вывода числа в консоль бесконечно
}
...
// То есть этот цикл иногда бывает необходим когда сначала надо выполнить тело 1 раз, а потом уже сделать проверку условия, и если условие true то выполнить 2-ой раз, а если false, то закончить цикл. Цикл while сначала проверяет, и может ни разу не выполнить тело. 
...
// Получили 6 строки в каждом по ключу:  name, Евген, age, 39, number, 89234076582 
...
arr_for_addition.forEach(function(item22, i22, array22){ // непонятно нихрена
