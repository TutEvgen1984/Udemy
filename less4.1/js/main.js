...
// T - это для разделения даты и времени.
...
// Миллисекунды не отображаются, но их можно потом извлечь из переменной

// Метод Dat.parse (parse англ. - разбор, синтаксический разбор, анализ):
...
// 6 это июнь, но у нас получился июль который должен быть 7 месяцем по счету. Это потому что месяца начинаются с нуля.
...
console.log(d4_month) // Получили: 6

// Метод getDate() - получить день месяца от 0 до 31. Не совсем логичное название, но его нужно запомнить:
...
// Язык как-то сам вычисляет день недели по своему внутреннему календарю высчитываю я 1970 года
...
// Метод getTime() - получить миллисекунды. Для заданной даты возвращает Timestamp 
...
// Метод setHours(20) - установка количества часов у существующей даты
d_4.setHours(20)
...
for (let i=1; i<=10; i++) { // сделать кол циклов 1000 для замеров
    console.log(0)
}

let end1 = + new Date ()

let circle_time = end1 - start1
console.log(circle_time)
// Получили: 31. И ещё 0 и рядом 1000, это означает 0 вывелось в консоль 1000 раз 
...
// Получили: 14 ровно - это столько же, сколько и в самодельном методе
...
function timeToMinute (time) { // При наведении на функцию подсказка "(time:any: void)" в которой "time: any" означает любой тип данных, ":void" означает что функция ничего не возвращает (пока). После прописывания @param {string} time получилось "(time:string: void)", то есть теперь не любой тип, а именно string (там может быть number, boolean). После добавления @return {number} получаем в подсказке "(time:string: number)": вместо void появился number что означает что вернется число. 
    try {
        ...
        let minute = +time.split(":")[1]
          if ( !(hour>=0 && hour<=23) || !(minute>=0 && minute<=59)) {// поставили ! знак отрицания потому что нам надо проверить что наши значения введены неправильно, || это ИЛИ что означает что одно любое из условий страбатывает и будет ошибка Или в часах или в минутах
...
// .split(":")[0] означает разделть на массив элементов, в качестве разделителя ":", [0] означает что взять из масcива элемент с индексом [0]. Если + и [0] не указать, то получим массив  "(2) ['02', '08']""
// После наведения курсора на функцию появляется всплывающая подсказка, в которой видно все что написано в этом новом виде комментариев /** */

console.log(timeToMinute("02:08"))
// Получили: 128  
// Получили: 2 потому что мы указали + и сконвертировали в число
// Если плюс не ставить, то получим "02", и это будет строка
// При наведении курсора на имя функции появляется подсказка. Эта функция может быть где-то далеко либо даже в другом файле, а при таком описании нам сразу видно её полное описание записанное в /** */ и станвится понятно что функция делает и как она работает, какие есть параметры, какого они типа. И что вернётся после return. 
...
// at timeToMinute (main.js:1690:18)
// at main.js:1709:13
...
// В нащем объекте попробуем вызвать метод toString()
user.toString()
// В консоли ничего не появилось
// А вот если написать в консоли user.toString(), то видим '[object Object]' красным цветом

// В нащем объекте попробуем вызвать метод valueOf()
user.valueOf()
// В консоли ничего не появилось
// А вот если написать в консоли user.valueOf(), то видим {name: 'Ivan', age: 20} красным цветом

// Эти 2 метода мы не создавали, но благодаря прототипу мы можем имим пользоваться.

let admin = {
    rules: 777,
    isAdmin() { // метод это обычная функция, но внутри какого-то объекта, и она позволяет делать что-то с его свойствами  
        console.log("Я админ " + this.name + ", мои права" + this.rules);
        // rules возьмётся из текущего объекта admin, name в данном объект нету, оно возьмется из дочернего объекта который унаследует текущий объект 
    }
}
...
// - name: "Ivan"
// - [[Prototype]]: Object (разворачивающийся список)
// --- rules: 777 // это свойство видно уже в развернувшейся веладке, его сразу не видно пока не развернёшь.
// здесь мы видим свойство rules, которое было унаследовано от нашего объекта admin, так как мы указали его в качестве прототипа для нашего объекта user
// --- [[Prototype]]: Object (разворачивающийся список)
...
// Получили [object Object]. Это мы проверили что так же до сих пор работает метод самого основного нашего родителя, который является прототипом для всех объектов, и мы так же можем олучить к нему доступ.

// ПРИНЦИП НАСЛЕДОВАНИЯ это следующее: Таким образом можно создать какой-то один какой-то комплексный объект, и потом создаём другие объекты и указываем в качестве прототипа наш большой родительский объект. И все дочерние объекты смогут иметь доступ к его свойствам и методам.

// Далее рассмотрим пример когда нам надо создавать много объектов по типу user.

// Далее ниже функция-конструктор
function User (name,age) { // имена функций конструкторов делаем с Большой буквы - это специально чтобы различать простые функции от конструкторов. Она будет работать и с маленькй буквы, но это принятое соглашение для удобства. Так будет удобно и нам и людям которые будус с нами работать
    ...
    this.age = age // значение age получаем из скобок метода, второе значение после запятой, которое мы сами будем передавать
...
let u1 = new User ("Petr", 21) // слово new это специальное слово, с помощью которого мы можем создать объект с помощью конструктора. Если new не написать, получаем в консоли main.js:1796 Uncaught TypeError: 
// Слово User это название конструктора
// В скобках мы передаём значения в качестве параметров name,age

console.log(u1);
// Получили: User {name: 'Petr', age: 21}
// А если раскрыть объект, то увидим дополнительную 3-ю строку [[Prototype]]: Object в которой находится rules:777
// Мы видим что u1 это объект User, потому что мы создавали его по конструктору User
...
console.log(u2);
// Получили: User {name: 'Kate', age: 38}

let u3 = new User ("Masha", 39)
console.log(u3);
// Получили: User {name: 'Masha', age: 39}

// new User это: new = ключевое слово - команда создать объект на основе конструктора, User = название конструктора

// В итоге получили 3 переменных u1, u2, u3 содержащие 3 объекта с одинаковым названием User и у каждого есть свои значения name, age, и так же каждый наследуется от admin и они по сути являются admin'ами.

// Как видно с помощью прототипов __proto__ можно наследовать какие-то свойства и методы от родительского объекта. А с помощью функции-конструктора можно создавать много однотипых объектов очень простым образом вместо того чтобы вручную создавать объекты и  разные значения им указывать. Там мы делали для даты - создавали много объектов даты с разными значениями и у нас создаются объекты для разных даты и времени.
...
// С помощью прототипов мы связываем один объект с другим можно использовать комбинацию свойств и методов нескольких объектов.
