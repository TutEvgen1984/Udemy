...
function ismail() {
    people.forEach(function (item) { // у нас 4 человека, поэтому метод сделает цикл на 4 итерации, каждую итерацию в качестве item у нас будет перебирающийся один из 4 объектов из массива people
        ...
        if (document.querySelector(`.items > div .item${i}`) == null) {
            ...
            // В CSS когда элемент p записывается БЕЗ пробела последовательно с классом, это называется комбинированный селектор: когда пишется сначала элемент, а после него без пробела  точка и имя класса p.class12, это означает что оформление применится только к элементу p имеющему класс class12, здесь комбинируется выбор элемента с выбором класса, поэтому называется комбинированным.
            // В нашем случае `div .item${i}` между элементом и классом  стоит пробел, это означает Селектор потомка: потомок ".item${i}" находится внутри предка в виде элемента div независимо от уровня вложенности (хоть первая вложенность, хоть 5-ая)
            // .items > div означает выбор ребенка (дочернего элемента) div, который находится внутри родителя в виде класса ".items", то есть вложенность максимум 1-го уровня.
